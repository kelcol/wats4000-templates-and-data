<template>
  <div class="results">
    ...
    <p class="search-meta">
      <span class="current-page"><b>Current Page:</b> </span> {{ page }}
      <span class="total-pages"><b>Pages:</b> </span> {{ total_pages }}
      <span class="total-results"><b>Count:</b> {{ total_results }}</span>
    </p>

    <ul>
      <li class="movie-item" v-for="result in results"> <!-- FIXME: [eslint-plugin-vue][vue/require-v-for-key]Elements in iteration expect to have 'v-bind:key' directives.-->
         <img v-bind:src="'https://image.tmdb.org/t/p/w150_and_h225_bestv2' + result.poster_path" v-bind:alt="result.original_title" class="poster-image">        
        <h2 class="title"><a v-bind:href="'https://www.themoviedb.org/movie/' + result.id">{{ result.original_title }}</a></h2>
        <div class="ratings">
        ...
        <ul class="genre-list">
          <li v-for="genre in result.genres">{{ genre }}</li> <!-- FIXME: [eslint-plugin-vue][vue/require-v-for-key]Elements in iteration expect to have 'v-bind:key' directives.-->
        </ul>
      </li>
    </ul>
  </div>
